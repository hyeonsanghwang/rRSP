...
def model_convolution_3():
    ...
    f = Conv2DTranspose(filters=32, kernel_size=3, strides=2, padding='same')(f)
    f = Conv2DTranspose(filters=32, kernel_size=3, strides=1, padding='same')(f)

    f = Concatenate()([f, lf3])

    f = Conv2DTranspose(filters=16, kernel_size=3, strides=2, padding='same')(f)
    f = Conv2DTranspose(filters=16, kernel_size=3, strides=1, padding='same')(f)

    f = Concatenate()([f, lf2])

    f = Conv2DTranspose(filters=8, kernel_size=3, strides=2, padding='same')(f)
    f = Conv2DTranspose(filters=8, kernel_size=3, strides=1, padding='same')(f)

    f = Concatenate()([f, lf1])

    f = Conv2DTranspose(filters=1, kernel_size=3, strides=2, padding='same')(f)
    f = Conv2DTranspose(filters=1, kernel_size=3, strides=1, padding='same')(f)
...
if __name__ == '__main__':
    # model = model_convolution_3()
    model = model_classification()
    # model.summary()
    speed_test(model, data_size=8, n_iter=1000)
    # signal = []
    # for i in range(1000):
    #     a = np.sin(i * np.pi / 100)
    #     signal.append(a)
    # np_signal = np.array(signal)
    # mask = np.hanning(100)
    # res = np.convolve(np_signal, mask)
    #
    # show_signal('s', signal)
    # show_signal('2', res)
    # cv2.waitKey(0)
