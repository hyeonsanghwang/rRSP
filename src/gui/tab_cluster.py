...
class TabCluster(QWidget, uic.loadUiType(ui_path('tab_cluster.ui'))[0]):
    """
    [Widgets]
    - Parameters
        spin_process_fps
    ...
    def __init__(self, params: ClusterParameter):
        ...
        self.b_flip.clicked.connect(lambda: self._on_clicked_function(self.b_flip))
        self.b_show_cluster.clicked.connect(lambda: self._on_clicked_function(self.b_show_cluster))
        self.b_show_symmetry_cluster.clicked.connect(lambda: self._on_clicked_function(self.b_show_symmetry_cluster))

        # Connect custom signal
        self.signal_changed_progress.connect(self.change_progress)
        self.params.set_signals(ClusterParameter.SIGNAL_CHANGE_PROGRESS, self.signal_changed_progress)
    ...
    def _on_clicked_show_mode(self, widget):
        if widget == self.b_show_original:
            self.params.show_mode = ClusterParameter.SHOW_MODE_ORIGINAL
        elif widget == self.b_show_resized:
            self.params.show_mode = ClusterParameter.SHOW_MODE_RESIZED
        elif widget == self.b_show_score:
            self.params.show_mode = ClusterParameter.SHOW_MODE_SCORE
        elif widget == self.b_show_roi:
            self.params.show_mode = ClusterParameter.SHOW_MODE_ROI
